  ...
  async configurePermissions(db, username, applicationName, permissions) {
    ...
    switch (permissions.write) {
      case 'users':
        writeUsers = _.union(
          writeUsers,
          Utils.didsToUsernames(permissions.writeList, applicationName)
        );
        deleteUsers = _.union(
          deleteUsers,
          Utils.didsToUsernames(permissions.deleteList, applicationName)
        );
        break;
      case 'public':
    ...
    switch (permissions.read) {
      case 'users':
        readUsers = _.union(
          readUsers,
          Utils.didsToUsernames(permissions.readList, applicationName)
        );
        break;
      case 'public':
    ...
    try {
      const writeFunction =
        ...
        writeUsersJson +
        ".indexOf(userCtx.name) == -1) throw({ unauthorized: 'User is not permitted to write to database' });\n}";
      const writeDoc = {
    ...
    if (permissions.write == 'public') {
      ...
      try {
        const deleteFunction =
          '\n    function(newDoc, oldDoc, userCtx, secObj) {\n        if (' +
          deleteUsersJson +
          ".indexOf(userCtx.name) == -1 && newDoc._deleted) throw({ unauthorized: 'User is not permitted to delete from database' });\n}";
        const deleteDoc = {
